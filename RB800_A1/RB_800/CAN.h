...
//IVA002: CAN Mutex
#define CAN_MX_LOCK()		cli()//TCD1.INTCTRLB = 0;PORTC_INT0MASK = 0;				// Halt interrupts
#define CAN_MX_UNLOCK()		sei()//TCD1.INTCTRLB = 0x01;PORTC_INT0MASK = 1<<CAN_INT;	// enable interrupts
...
uint8_t data_length, uint8_t data[]);
//void transmit_tx1_CAN(uint16_t standard_identifier, uint8_t remote_transmission_request,
//	uint8_t data_length, uint8_t data[]);
//void transmit_tx2_CAN(uint16_t standard_identifier, uint8_t remote_transmission_request,
//	uint8_t data_length, uint8_t data[]);
void reset_CAN_error(void);
void stop_CAN(void);

volatile uint8_t can_flag;
volatile uint8_t can_failure;
