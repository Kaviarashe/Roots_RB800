...
//Turns on the lcd cursor (no blinking)
...
{
	uint8_t i = 0;
	//Temp array for printing menu
	char m[11];

	lcd_clear_screen();

	//Print slow/fast status
	...
	else
	{
		if (fr_speed == SLOW)
		{
			lcd_write_new_P(PSTR("SLOW"));
		}
		else
		{
			lcd_set_cursor(1, 0);
			lcd_write_P(PSTR("FAST"));
		}
		...
		if (tank_full)
		{
			lcd_set_cursor(0, 5);
			i = 0;
			m[i++] = TANK_FULL_LEFT_LCD;
			m[i++] = TANK_FULL_RIGHT_LCD;
			m[i++] = '\0';
			lcd_write(m);

			lcd_write_P(PSTR("TANK"));
			lcd_set_cursor(1, 7);
			lcd_write_P(PSTR("FULL"));
		}
	}
	...
	lcd_set_cursor(0, 12);
	if (colon_toggle || !runtimer_on)
	{
		sprintf(m, "%5d:%02d", hours, minutes);
	}
	else
	{
		sprintf(m, "%5d %02d", hours, minutes);
	...
	else
	{
		...
		for (j = 0; j < 5; j++)
		{
			if (min_battery_level == 0)
				m[i++] = ' ';
			else if ((min_battery_level / 20) >= j)
				m[i++] = BAT_MID_LCD;
			else
				m[i++] = ' ';
		}
...
//Updates part of screen rather than entire screen to reduce screen flicker
